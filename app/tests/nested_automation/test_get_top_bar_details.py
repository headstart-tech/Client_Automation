"""
This file contains test cases related to get automation top bar details.
"""
import pytest
...
async def test_automation_top_bar_details(
        ...
        start_end_date):
    """
    Different test cases of get automation top bar details.
    ...
    # College id not found to get automation top bar details
    response = await http_client_test.post(
        f"/nested_automation/top_bar_details/"
    ...
    # College id invalid to get automation top bar details
    response = await http_client_test.post(
        f"/nested_automation/top_bar_details/"
    ...
    # No permission to get automation top bar details
    response = await http_client_test.post(
        f"/nested_automation/top_bar_details/"
        f"?college_id={college_id}&automation_id={automation_id}&feature_key={feature_key}",
        headers={"Authorization": f"Bearer {access_token}"})
    ...
    # Get automations top bar details
    response = await http_client_test.post(
        f"/nested_automation/top_bar_details/"
        f"?college_id={college_id}&automation_id={automation_id}&feature_key={feature_key}",
        headers={
    ...
    assert response.json()['message'] == "Get the automation top bar details."
    for item in ["status", "count_at_origin", "date", "time",
                 "current_data_count", "email_communication",
                 "sms_communication", "whatsapp_communication"]:
        assert item in response.json()["data"]
    assert response.json()["data"]["automation_name"] == "test"
    assert response.json()["data"]["data_type"] is None

    # Get automations top bar details by date_range
    response = await http_client_test.post(
        f"/nested_automation/top_bar_details/"
        f"?college_id={college_id}&automation_id={automation_id}&feature_key={feature_key}",
        headers={
    ...
    # Automation id required to get automation top bar details
    response = await http_client_test.post(
        f"/nested_automation/top_bar_details/?"
    ...
    # Automation id not found to get automation top bar details
    response = await http_client_test.post(
        f"/nested_automation/top_bar_details/"
    ...
    # Automation id invalid to get automation top bar details
    response = await http_client_test.post(
        f"/nested_automation/top_bar_details/"
