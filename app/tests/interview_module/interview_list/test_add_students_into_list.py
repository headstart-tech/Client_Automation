...
@pytest.mark.asyncio
async def test_add_student_into_interview_list(http_client_test,
    ...
    assert response.json() == {
        "detail": "Interview List Id must be required and valid."}
    ...
    assert response.json() == {"message": "Students added into interview list."}

    # Required college id for delete students from interview list
    response = await http_client_test.post(
        f"/interview/add_students_into_list/?interview_list_id={application_id}&feature_key={feature_key}",
        headers={"Authorization": f"Bearer {college_super_admin_access_token}"},
        json=[application_id]
    )
    assert response.status_code == 400
    ...
    response = await http_client_test.post(
        f"/interview/add_students_into_list/?college_id=1234567890&interview_list_id={application_id}"
        f"&feature_key={feature_key}",
        headers={
            "Authorization": f"Bearer {college_super_admin_access_token}"},
        json=[application_id]
    ...
    response = await http_client_test.post(
        f"/interview/add_students_into_list/?college_id=123456789012345678901234&"
        f"interview_list_id={interview_list_id}&feature_key={feature_key}",
        headers={
            "Authorization": f"Bearer {college_super_admin_access_token}"},
        json=[application_id]
    ...
    assert response.status_code == 422
    assert response.json()['detail'] == 'Interview list id must be a ' \
    ...
    #        "Interview list not found. Make sure provided interview list" \
