"""
This file contains test cases of get user reports.
"""
import pytest
...
@pytest.mark.asyncio
async def test_current_user_report(
    ...
    """
    Different test cases of get current user reports.
    ...
    Params:\n
        ...
        - test_schedule_report_validation: A fixture which delete all existing
            reschedule reports and create a new rescheduling report.
    ...
    response = await http_client_test.post(
        f"/reports/current_user/?college_id={college_id}&feature_key={feature_key}",
        headers={"Authorization": "wrong bearer"}
    )
    assert response.status_code == 401
    assert response.json()["detail"] == "Not authenticated"

    # Bad token for get reports of current user
    response = await http_client_test.post(
        f"/reports/current_user/?college_id={college_id}&feature_key={feature_key}",
    ...
    # Required page number for get reports of current user
    response = await http_client_test.post(
        f"/reports/current_user/?college_id={college_id}&feature_key={feature_key}",
    ...
    # Required page size for get reports of current user
    response = await http_client_test.post(
        f"/reports/current_user/?page_num=1&college_id={college_id}&feature_key={feature_key}",
        headers={"Authorization": f"Bearer "
                                  f"{college_super_admin_access_token}"}
    )
    assert response.status_code == 400
    ...
    # Get reports of current user
    response = await http_client_test.post(
        f"/reports/current_user/?page_num=1&page_size=100&college_id="
        f"{college_id}&feature_key={feature_key}",
        headers={"Authorization": f"Bearer "
                                  f"{college_super_admin_access_token}"},
    )
    assert response.status_code == 200
    assert response.json()["message"] == "Get reports of current user."
    await validate_successful_result(response)

    # Get current user reports by search pattern
    response = await http_client_test.post(
        f"/reports/current_user/?page_num=1&page_size=100&college_id="
    ...
    # Get current user reports not found by search pattern
    response = await http_client_test.post(
        f"/reports/current_user/?page_num=1&page_size=100&college_id="
    ...
    # All reports of current user not found
    ...
    response = await http_client_test.post(
        f"/reports/current_user/?page_num=1&page_size=100&"
    ...
    # Get reschedule reports of current user
    response = await http_client_test.post(
        f"/reports/current_user/?page_num=1&page_size=100&college_id="
        f"{college_id}&reschedule_report=true&feature_key={feature_key}",
        headers={"Authorization": f"Bearer "
                                  f"{college_super_admin_access_token}"}
    )
    await validate_successful_result(response)

    # Get current user reschedule reports by search pattern
    response = await http_client_test.post(
        f"/reports/current_user/?page_num=1&page_size=100&college_id="
    ...
    # Get current user reschedule reports not found by search pattern
    response = await http_client_test.post(
        f"/reports/current_user/?page_num=1&page_size=100&college_id="
    ...
    # Reschedule reports not found for current user
    await DatabaseConfiguration().report_collection.delete_many(
    ...
    # Get current user auto schedule reports
    response = await http_client_test.post(
        f"/reports/current_user/?page_num=1&page_size=100&college_id="
    ...
    # Get current user auto schedule reports by search pattern
    response = await http_client_test.post(
        f"/reports/current_user/?page_num=1&page_size=100&college_id="
        f"{college_id}&auto_schedule_reports=true&"
        f"search_pattern=auto_reschedule&feature_key={feature_key}",
        headers={"Authorization": f"Bearer "
                                  f"{college_super_admin_access_token}"}
    )
    await validate_successful_result(response)

    # Auto schedule reports not found for current user by search_pattern
    await DatabaseConfiguration().report_collection.delete_many(
