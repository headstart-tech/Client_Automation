...
@pytest.mark.asyncio
async def test_list_of_search_user_based_on_matched_string_not_authenticated(
    ...
    response = await http_client_test.get(
        f"/admin/search_students/?search_input=temp&"
...
@pytest.mark.asyncio
async def test_list_of_search_user_based_on_matched_string_no_permission(
    ...
    """
    response = await http_client_test.get(
        f"/admin/search_students/?page_num=1&page_size=20&search_input=temp&"
...
#     """
#     response = await http_client_test.get(
#         f"/admin/search_students/?search_input={test_student_validation['user_name']}&page_num=1&page_size=1&college_id={str(test_student_validation.get('college_id'))}",
#         headers={"Authorization": f"Bearer {college_super_admin_access_token}"},
#     )
#     assert response.status_code == 200
#     assert response.json()["message"] == "Fetched student_id and application_id successfully"
#     for item in ["data", 'count', "page_num"]:
...
async def test_search_of_username_when_search_string_not_provided(
    ...
    """
    response = await http_client_test.get(
        f"/admin/search_students/?search_string=&page_num=1&page_size=1&"
        f"college_id={str(test_college_validation.get('_id'))}&feature_key={feature_key}",
        headers={"Authorization": f"Bearer {college_super_admin_access_token}"},
    )
