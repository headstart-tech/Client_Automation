"""
Test case for all the call data set in header section of QA manager.
"""
...
async def test_unauthenticated_get_data(
    ...
    response = await http_client_test.post(
        f"/qa_manager/call_list_metrics/?feature_key={feature_key}"
    )
...
async def test_invalid_college_id(http_client_test, setup_module, college_super_admin_access_token):
    ...
    # Invalid college id.
    response = await http_client_test.post(
        f"/qa_manager/call_list_metrics/?college_id="
...
@pytest.mark.asyncio
async def test_get_valid_data_set_with_date_range(http_client_test, setup_module, college_super_admin_access_token, test_college_validation, start_end_date):
