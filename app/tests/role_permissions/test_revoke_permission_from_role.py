"""
Unit tests for the `revoke_permissions` endpoint.

This module validates the `revoke_permissions` API behavior under different scenarios:
...
2. **Invalid Data Handling**:
   - Invalid role IDs (e.g., None, malformed, or non-existent).
...
3. **Valid Data Handling**:
   - Super Admin:
     - Can revoke valid permissions for a role.
...
fetch_latest_role_perm : Role
    Fixture providing the latest role with permissions.
role_id : Any
    The role identifier (valid, invalid, or None).
user_request : Any
...
- Input validation (role ID and payload).
...
async def test_revoke_permission_without_token(http_client_test):
    ...
    response = await http_client_test.post(
        f"/role_permissions/revoke_permissions/1?feature_key={feature_key}",
...
# Todo: Need to add updated testcases for revoke_permissions from role
...
#
#     response = await http_client_test.post(
#         "/role_permissions/revoke_permissions/1",
...
# ):
#     """
#     Test the 'revoke_permissions' endpoint with invalid data.
#
...
#     - role_id : Any - Role ID (invalid or None).
...
#     response = await http_client_test.post(
#         f"/role_permissions/revoke_permissions/{role_id}",
#         headers={"Authorization": f"Bearer {super_admin_token}"},
#         json=user_request,
#     )
...
# ):
#     """
#     Test the 'revoke_permissions' endpoint.
#
#     Validates permission revocation for a role under different scenarios:
#
#     1. Super Admin:
...
#     perm_data = await fetch_assigned_permission_ids("role", role_id)
#
